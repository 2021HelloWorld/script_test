...
这套部署脚本是宿主机侧的辅助脚本，不是 easim 仿真项目源码的一部分。可以把 `script_one_in_all/` 放在其他宿主机目录中使用，只要按实际位置执行脚本，并确保 `config.sh` 中的 `EASIM_HOST_PATH` 指向 easim 仿真项目根目录即可。容器启动时会把 `EASIM_HOST_PATH` 挂载为 `/easim`，把当前脚本目录挂载为 `/deploy_scripts`。
...
```bash
bash script_one_in_all/00_install_cuda.sh
# 验证
nvcc --version
```
...
```bash
bash script_one_in_all/01_install_host_deps.sh
# 验证
docker --version
tmux -V
```
...
```bash
bash script_one_in_all/easim.sh
```

常用快捷命令：

```bash
bash script_one_in_all/easim.sh check    # 环境预检，只报告状态
bash script_one_in_all/easim.sh deploy   # 首次部署
bash script_one_in_all/easim.sh restart  # 重启/恢复环境
bash script_one_in_all/easim.sh teleop   # 启动遥操
bash script_one_in_all/easim.sh process  # 数据处理
bash script_one_in_all/easim.sh status   # 状态查看
```
...
```bash
bash script_one_in_all/02_setup_cdi.sh
```
...
```bash
bash script_one_in_all/03_build_image.sh
```
...
5. 以 CDI GPU 直通方式启动容器，同时挂载两个目录：
   ...
   - `script_one_in_all/ → /deploy_scripts`（本目录下的部署脚本）
...
```bash
bash script_one_in_all/04_start_container.sh
```
...
> 容器内 `/deploy_scripts/` 对应宿主机的 `script_one_in_all/` 目录，`/easim/scripts/` 仍为 easim 仓库原有脚本，互不干扰。
...
```bash
bash script_one_in_all/06_teleop_pico.sh
```
...
```bash
bash script_one_in_all/07_process_dataset.sh
```
...
bash script_one_in_all/04_start_container.sh  # 选 2 重建容器
```
