...
load_config() {
    [ -f "$CONFIG_FILE" ] || error "未找到 config.sh，请先运行：bash scripts/easim.sh setup"
...
check_cuda_toolkit() {
    ...
    if ! command -v nvcc &>/dev/null; then
        if [ -x "$EXPECTED_CUDA_NVCC" ]; then
            ...
            check_hint "建议检查 ${EXPECTED_CUDA_NVCC}，必要时执行：bash scripts/easim.sh cuda"
            return
        else
            check_fail "CUDA Toolkit：未找到 nvcc"
            check_hint "建议执行：bash scripts/easim.sh cuda"
        fi
        return
    fi
...
check_docker_env() {
    ...
    if ! command -v docker &>/dev/null; then
        ...
        check_hint "建议执行：bash scripts/easim.sh host-deps"
        return
    fi
    ...
    if [ -z "$docker_version" ] || ! [[ "$docker_major" =~ ^[0-9]+$ ]]; then
        ...
        check_hint "CDI 依赖 Docker >= ${MIN_DOCKER_MAJOR}.0，建议执行：bash scripts/easim.sh host-deps"
...
check_tmux_env() {
    if command -v tmux &>/dev/null; then
        ...
        check_hint "部署可继续，但遥操脚本依赖 tmux；建议执行：bash scripts/easim.sh host-deps"
    fi
}
...
check_config_paths() {
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        check_hint "建议执行：bash scripts/easim.sh setup"
        return
    fi
    ...
    if [ -n "${EASIM_HOST_PATH:-}" ] && [ -d "$EASIM_HOST_PATH" ]; then
        ...
        check_hint "请执行：bash scripts/easim.sh setup"
    fi
...
first_deploy() {
    ...
    if ask_yes_no "是否现在初始化容器环境？新容器通常需要执行一次。" "yes"; then
        ...
        info "稍后可运行：bash scripts/easim.sh init"
    fi
}
