...
usage() {
    cat <<EOF
用法:
  bash script_set_env/easim.sh [command]
...
load_config() {
    [ -f "$CONFIG_FILE" ] || error "未找到 config.sh，请先运行：bash script_set_env/easim.sh setup"
...
check_config_paths() {
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        check_hint "建议执行：bash script_set_env/easim.sh setup"
        return
    fi
    ...
    if [ -n "${EASIM_HOST_PATH:-}" ] && [ -d "$EASIM_HOST_PATH" ]; then
        ...
        check_hint "请执行：bash script_set_env/easim.sh setup"
    fi
...
first_deploy() {
    ...
    if ask_yes_no "是否现在初始化容器环境？新容器通常需要执行一次。" "yes"; then
        ...
        if ask_yes_no "是否进行 Isaac Sim / Isaac Lab 环境验证？" "yes"; then
            ...
            info "稍后可运行：bash script_set_env/easim.sh verify"
        fi
    else
        info "稍后可运行：bash script_set_env/easim.sh init"
